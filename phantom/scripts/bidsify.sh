...
dcm2niix -o "$anat" -f sub-"$sub"_acq-R1opt_echo-%e_flip-2_mt-off_MPM "$indir"/S6_t1w_*_R1
dcm2niix -o "$anat" -f sub-"$sub"_acq-R1opt_echo-%e_flip-1_mt-off_MPM "$indir"/S7_pdw_*_R1
dcm2niix -o "$anat" -f sub-"$sub"_acq-PDopt_echo-%e_flip-2_mt-off_MPM "$indir"/S8_t1w_*_PD1
dcm2niix -o "$anat" -f sub-"$sub"_acq-PDopt_echo-%e_flip-1_mt-off_MPM "$indir"/S10_pdw_*_PD
...
# AFI
dcm2niix -o "$fmap" -f sub-"$sub"_acq-tr%e_TB1AFI "$indir"/S2_kp_afib1_*
...
# SESTE
dcm2niix -o "$fmap" -f sub-"$sub"_echo-%e_TB1EPI "$indir"/S3_kp_seste_b1map_*
te=0.03906
tm=0.0338
...
# B0 for SESTE
dcm2niix -o "$fmap" -f sub-"$sub"_magnitude%e "$indir"/S4_gre_field_map_*
dcm2niix -o "$fmap" -f sub-"$sub"_phasediff "$indir"/S5_gre_field_map_*
